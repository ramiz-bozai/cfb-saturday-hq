...
| Simulator execution | Completed wins and conference wins are computed once as indexed NumPy vectors. Remaining-game probabilities are resolved once, all random outcomes are drawn as one matrix, and win accumulation is vectorized. Finished seasons automatically use one deterministic pass. A fixed notebook seed makes identical inputs reproducible. On the completed 2025 schedule, the local simulation core fell from 24.493 seconds for 2,000 redundant passes to 0.000706 seconds for one equivalent pass, with the same 12-team field |
